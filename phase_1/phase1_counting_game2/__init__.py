...
class C(BaseConstants):
    NAME_IN_URL = 'phase1_counting_game2'
...
class SetGoal(Page):
    form_model = "player"
    form_fields = ["goal_set", "goal_set_amount"]
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        participant.expiry = time.time() + 2*60 # change 0.5 back to ten

        if player.goal_set == True:
            # player.goal_set_amount = 20 this may be different according to the player input
            player.goal_left = player.goal_set_amount
...
class CombinedResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        player.combined_payoff = participant.combined_payoff
